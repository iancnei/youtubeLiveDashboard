(function() {
  ...
  function getUserChannel() {
    ...
    request.execute(function(response) {
      if ('error' in response) {
        ...
        getLiveBroadcastItems();
      }
    });
  }

  // Call the Data API to retrieve items from past Live Broadcasts.
  function getLiveBroadcastItems() {
    ...
    request.execute(function(response) {
      if ('error' in response) {
        ...
        if ('items' in response) {
          var liveBroadcastList = $('#live-broadcast-list');
          $.each(response.items, function() {
            // Exclude liveBroadcasts that do not have any views, since those liveBroadcasts
            // will not have any interesting viewcount Analytics data.
            var title = this.snippet.title;
            var description = this.snippet.description;
            var liveBroadcastId = this.id;

            // Create a new <li> element that contains an <a> element.
            // Set the <a> element's text content to the liveBroadcast's title, and
            ...
            aElement.click(function() {
              displayLiveBroadcastAnalytics(liveBroadcastId);
            });

            // Call the jQuery.append() method to add the new <a> element to
            // the <li> element, and the <li> element to the parent
            // list, which is identified by the 'liveBroadcastList' variable.
            liElement.append(aElement);
            liveBroadcastList.append(liElement);
          });

          if (liveBroadcastList.children().length == 0) {
            // Display a message if the channel does not have any viewed liveBroadcasts.
            displayMessage('Your channel does not have any live broadcasts that have been viewed.');
          }
        } else {
          displayMessage('There are no live broadcasts in your channel.');
        }
      }
    })
  }
  ...
  // This function doesn't work correctly.  The request goes through fine, but nothing is in the response.
  function displayLiveBroadcastAnalytics(liveBroadcastId) {
    if (channelId) {
      var today = new Date();

      var request = gapi.client.youtubeAnalytics.reports.query({
        'start-date': formatDateString(today),
        'end-date': formatDateString(today),
        ids: "channel==" + channelId,
        dimensions: 'day',
        sort: 'day',
        metrics: 'views,comments,averageViewDuration,likes,shares,subscribersGained',
        filters: 'video==' + liveBroadcastId
      });
      ...
      request.execute(function(response) {
        ...
        if ('error' in response) {
          ...
          displayChart(liveBroadcastId, response);
        }
      });
    } else {
